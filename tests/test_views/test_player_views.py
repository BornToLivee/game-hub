...
from game.models import Player, Genre, Platform, Publisher
...
class PlayerUpdateViewTestCase(TestCase):
    ...
    def test_user_can_successfully_submit_registration_form(self):
        client = Client()
        response = client.post(reverse('game:register'), {
...
class PlayerUpdateViewTests(TestCase):
    ...
    def test_successful_update(self):
        response = self.client.post(reverse("game:player-update"), {
            'first_name': 'John',
            'last_name': 'Doe',
    ...
    def test_redirects_on_success(self):
        ...
        request = factory.post('/player/update/', {
            'first_name': 'John',
            'last_name': 'Doe',
            'date_of_birth': '1990-01-01'
        })
    ...
    def test_player_update_view_with_current_user_data(self):
        client = self.client
        player = get_user_model().objects.create_user(username='testuser', password='12345')
...
class PersonalPageViewTestCase(TestCase):
    def setUp(self):
        ...
        self.publisher = Publisher.objects.create(name='Test Publisher', description='Test Publisher', country='Ukraine', capitalization=0.02)
    ...
    def test_user_with_wishlist_and_completed_games_sees_paginated_lists(self):
        ...
        user.wishlist_games.all.return_value = ['game1', 'game2', 'game3', 'game4', 'game5', 'game6']
        user.completed_games.all.return_value = ['game1', 'game2', 'game3', 'game4', 'game5', 'game6']
