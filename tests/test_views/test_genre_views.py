...
class GenreDetailViewTestCase(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            username="testuser", password="12345", email="qwe@qwe.com"
        )
    ...
    def test_genre_detail_view_returns_correct_context_data(self):
        ...
        response = self.client.get(
            reverse("game:genre-detail", kwargs={"pk": self.genre.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_genre_detail_view_handles_no_associated_games(self):
        ...
        response = self.client.get(
            reverse("game:genre-detail", kwargs={"pk": genre.pk})
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("games", response.context_data)
    ...
    def test_genre_detail_view_contains_update_and_delete_links(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, reverse("game:genre-update", kwargs={"pk": self.genre.pk})
        )
        self.assertContains(
            response, reverse("game:genre-delete", kwargs={"pk": self.genre.pk})
        )
...
class GenreDeleteViewTestCase(TestCase):
    ...
    def test_delete_genre_authenticated_and_redirected_correctly(self):
        ...
        response = self.client.post(
            reverse("game:genre-delete", kwargs={"pk": genre.id})
        )
...
class GenresUpdateViewTestCase(TestCase):
    ...
    def test_update_view_uses_correct_template(self):
        response = self.client.get(
            reverse("game:genre-update", kwargs={"pk": self.genre.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_successful_update(self):
        ...
        self.assertRedirects(
            response, reverse("game:genre-detail", kwargs={"pk": self.genre.pk})
        )
...
class GenreCreateViewTestCase(TestCase):
    ...
    def test_create_genre_with_valid_data(self):
        valid_data = {"name": "Adventure", "description": "Adventure games"}
    ...
    def test_create_genre_with_invalid_data(self):
        invalid_data = {"name": "", "description": "Adventure games"}
