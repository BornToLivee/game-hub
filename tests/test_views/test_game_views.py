...
class GameCreateViewTestCase(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            username="testuser", password="sadfawsfsdaf"
        )
        ...
        self.publisher = Publisher.objects.create(
            name="Test Publisher",
            description="Test Publisher desc",
            country="USA",
            capitalization=0.02,
        )
...
class GameDeleteViewTestCase(TestCase):
    ...
    def test_successful_deletion_logged_in_user(self):
        ...
        response = self.client.post(
            reverse("game:game-delete", kwargs={"pk": self.game.pk})
        )
...
class GameDetailViewTestCase(TestCase):
    ...
    def test_calculate_unique_user_votes(self):
        ...
        response = self.client.get(
            reverse("game:game-detail", kwargs={"pk": self.game.pk})
        )
